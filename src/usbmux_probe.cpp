...
static bool ResolveUsbExport(DWORD ordinal, void** out) {
    it360_platform::ResolveStatus detail;
    if (it360_platform::ResolveModuleOrdinal("xboxkrnl.exe", ordinal, out, &detail)) return true;
    if (detail.has_status) {
        DbgPrint("[iPhoneTether360:B9C] USB export resolve failed ordinal=%u operation=%s NTSTATUS=0x%08x\n",
                 static_cast<unsigned>(ordinal), it360_platform::ResolveOperationName(detail.operation),
                 static_cast<unsigned>(detail.status));
    } else {
        DbgPrint("[iPhoneTether360:B9C] USB export resolve failed ordinal=%u operation=%s status=unavailable\n",
                 static_cast<unsigned>(ordinal), it360_platform::ResolveOperationName(detail.operation));
    }
    return false;
}
...
static bool ResolveUsbExports() {
    ...
    bool ok = true;
    if (!ResolveUsbExport(740, (void**)&gUsb.addDeviceComplete)) ok = false;
    if (!ResolveUsbExport(742, (void**)&gUsb.getDeviceSpeed)) ok = false;
    if (!ResolveUsbExport(744, (void**)&gUsb.getEndpointDescriptor)) ok = false;
    if (!ResolveUsbExport(746, (void**)&gUsb.openDefaultEndpoint)) ok = false;
    if (!ResolveUsbExport(747, (void**)&gUsb.openEndpoint)) ok = false;
    if (!ResolveUsbExport(748, (void**)&gUsb.queueAsyncTransfer)) ok = false;
    if (!ResolveUsbExport(749, (void**)&gUsb.queueCloseDefaultEndpoint)) ok = false;
    if (!ResolveUsbExport(750, (void**)&gUsb.queueCloseEndpoint)) ok = false;
    if (!ResolveUsbExport(751, (void**)&gUsb.removeDeviceComplete)) ok = false;
    if (!ResolveUsbExport(759, (void**)&gUsb.getDeviceDescriptor)) ok = false;
...
static void FailStatus(const char* why, const char* operation, uint32_t status) {
    gCtx.state = ProbeFailed;
    it360_link::SetPairReady(false);
    DbgPrint("[iPhoneTether360:B9C] FAIL %s | operation=%s status=0x%08x\n",
...
static int ControlComplete(UsbTrb* trb, int status) {
    ...
    if (status != 0) {
        FailStatus("SET_CONFIGURATION completion error", "USB control completion", static_cast<uint32_t>(status));
        return status;
    }
...
static int QueueSetConfiguration() {
    ...
    if (q < 0) FailStatus("queue SET_CONFIGURATION failed", "UsbdQueueAsyncTransfer", static_cast<uint32_t>(q));
    return q;
}
...
static int TxComplete(UsbTrb* trb, int status) {
    ...
    if (status != 0) {
        gCtx.txBusy = false;
        FailStatus("Bulk OUT completion error", "USB Bulk OUT completion", static_cast<uint32_t>(status));
        return status;
    }
    gCtx.txBusy = false;
    PumpTx();
...
static void PumpTx() {
    ...
    if (q < 0) {
        gCtx.txBusy = false;
        FailStatus("queue Bulk OUT failed", "UsbdQueueAsyncTransfer", static_cast<uint32_t>(q));
    }
}
...
static void StartTrustRetry() {
    ...
    it360_platform::ThreadStartStatus thread_status;
    if (!it360_platform::StartDetachedThread(
            TrustRetryThread, reinterpret_cast<void*>(static_cast<uintptr_t>(generation)), &thread_status)) {
        it360_platform::AtomicExchange(&gTrustRetryActive, 0);
        if (thread_status.has_status)
            DbgPrint("[iPhoneTether360:B9C] Pair trust retry worker failed operation=%s NTSTATUS=0x%08x\n",
                     it360_platform::ThreadStartOperationName(thread_status.operation),
                     static_cast<unsigned>(thread_status.status));
        else
            DbgPrint("[iPhoneTether360:B9C] Pair trust retry worker failed operation=%s status=unavailable\n",
                     it360_platform::ThreadStartOperationName(thread_status.operation));
        Fail("could not start Pair trust retry worker");
        return;
    }
    if (thread_status.close_status_valid && it360_platform::FailedStatus(thread_status.close_status))
        DbgPrint("[iPhoneTether360:B9C] Pair trust retry worker NtClose(handle) failed NTSTATUS=0x%08x\n",
                 static_cast<unsigned>(thread_status.close_status));
#endif
}
...
static int RxComplete(UsbTrb* trb, int status) {
    ...
    if (status != 0) {
        FailStatus("Bulk IN completion error", "USB Bulk IN completion", static_cast<uint32_t>(status));
        return status;
    }
...
static int QueueRx() {
    ...
    if (q < 0) FailStatus("queue Bulk IN failed", "UsbdQueueAsyncTransfer", static_cast<uint32_t>(q));
    return q;
}
...
static int StartMuxTransport() {
    ...
    if (it360_platform::FailedStatus(st)) {
        FailStatus("open usbmux Bulk IN failed", "UsbdOpenEndpoint(IN)", static_cast<uint32_t>(st));
        return st;
    }
    st = gUsb.openEndpoint(gCtx.handle, kBulkType, gCtx.outAddress,
                           ...
                           (DWORD*)&gCtx.bulkOut);
    if (it360_platform::FailedStatus(st)) {
        FailStatus("open usbmux Bulk OUT failed", "UsbdOpenEndpoint(OUT)", static_cast<uint32_t>(st));
        return st;
    }
...
static int AddDevice(DeviceHandle* handle) {
    ...
    if (it360_platform::FailedStatus(st) || !gCtx.control.trb.endpoint) {
        FailStatus("open EP0 failed", "UsbdOpenDefaultEndpoint", static_cast<uint32_t>(st));
        return st;
    }
...
static int RemoveDevice(DeviceHandle* handle) {
    ...
    if (handle && gCtx.bulkIn.endpoint) {
        ...
        if (it360_platform::FailedStatus(status))
            DbgPrint("[iPhoneTether360:B9C] UsbdQueueCloseEndpoint(IN) failed NTSTATUS=0x%08x\n",
                     static_cast<unsigned>(status));
    }
    if (handle && gCtx.bulkOut.endpoint) {
        const NTSTATUS status = gUsb.queueCloseEndpoint(handle, &gCtx.bulkOut);
        if (it360_platform::FailedStatus(status))
            DbgPrint("[iPhoneTether360:B9C] UsbdQueueCloseEndpoint(OUT) failed NTSTATUS=0x%08x\n",
                     static_cast<unsigned>(status));
    }
    if (handle && gCtx.control.trb.endpoint) {
        const NTSTATUS status = gUsb.queueCloseDefaultEndpoint(handle, &gCtx.control);
        if (it360_platform::FailedStatus(status))
            DbgPrint("[iPhoneTether360:B9C] UsbdQueueCloseDefaultEndpoint failed NTSTATUS=0x%08x\n",
                     static_cast<unsigned>(status));
    }
    ...
    if (handle) {
        ...
        if (it360_platform::FailedStatus(status))
            DbgPrint("[iPhoneTether360:B9C] UsbdRemoveDeviceComplete failed NTSTATUS=0x%08x\n",
                     static_cast<unsigned>(status));
    }
    memset(&gCtx, 0, sizeof(gCtx));
    return 0;
